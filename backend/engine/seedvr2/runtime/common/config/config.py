...
from backend.engine.seedvr2.runtime.common.schedulers import SCHEDULER_REGISTRY
...
class Config:
    def __init__(
        ...
        if width is None or height is None:
            raise ValueError(
                "SeedVR2 upscale requires explicit width and height; dynamic resolution is not wired in DanQing."
            )
    ...
    def scheduler(self):
        ...
        registered = SCHEDULER_REGISTRY.get(self._scheduler_str)
        if registered is None:
            raise NotImplementedError(
                f"The scheduler {self._scheduler_str!r} is not available for SeedVR2. "
