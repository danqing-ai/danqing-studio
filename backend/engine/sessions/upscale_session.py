"""UpscaleSession — image upscale orchestration."""

from __future__ import annotations

from typing import Any, Callable

from backend.core.contracts import ExecutionContext, ImageUpscaleRequest
from backend.engine.pipelines.upscale_pipeline import UpscalePipeline
from backend.engine.sessions.media_session import MediaSession
from backend.engine.sessions.phased_create import run_upscale_create_phased
from backend.engine.sessions.session_routing import routes_with_plugin_and_action


def routes_to_upscale_session(model_field: str, registry: Any) -> bool:
    return routes_with_plugin_and_action(
        model_field,
        registry,
        expected_media="image",
        api_action="upscale",
    )


class UpscaleSession(MediaSession):
    """v3 upscale — resolve plugin, then phased ``run_upscale_create``."""

    media_label = "upscale"

    def _make_pipeline(self) -> UpscalePipeline:
        return UpscalePipeline(
            self._runtime_ctx,
            self._registry,
            self._asset_store,
            model_cache=self._cache,
            project_root=self._project_root,
        )

    def run(
        self,
        request: ImageUpscaleRequest,
        exec_ctx: ExecutionContext,
        *,
        on_progress: Callable | None = None,
        on_log: Callable | None = None,
    ) -> Any:
        resolved, pipeline, log_cb = self._prepare(request, exec_ctx, on_log)
        return run_upscale_create_phased(
            pipeline,
            resolved,
            request,
            exec_ctx,
            on_progress=on_progress,
            on_log=log_cb,
        )
