...
def remap_flux2_weights(weights: dict) -> dict:
    ...
    DanQing structure matches reference, only minor remapping needed:
    - .to_out.0 → .to_out (remove Sequential wrapper)
    - .to_add_out.0 → .to_add_out (same)
    - time_guidance_embed.timestep_embedder. → time_guidance_embed. (remove wrapper layer)
    - Skip norm_added_q/norm_added_k (DanQing reuses norm_q/norm_k)
    """
    remapped = {}
