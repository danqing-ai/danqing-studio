"""Flux.2 Klein — MLX-only LoRA merge into loaded transformer linear weights."""
...
def merge_flux2_lora_adapters(
    ...
    *,
    ...
    if not hasattr(model, "_param_map"):
        raise RuntimeError("Flux2 LoRA merge requires a TransformerBase-style model with ``_param_map``.")
    ...
    for item in adapters:
        ...
        groups = remap_flux2_lora_keys(weights)
        if not groups:
            raise RuntimeError(
