...
def build_esrgan_plugin(
    ...
    *,
    ...
    config = get_config_class("esrgan")()
    spec = family_spec_from_config("esrgan", config, media="image")
    return FamilyPlugin(family_id="esrgan", spec=spec, backbone=UpscalePluginBackbone(spec))


def register_esrgan_plugin() -> None:
    register_family("esrgan", build_esrgan_plugin)
