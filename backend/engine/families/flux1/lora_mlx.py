"""Flux.1 — MLX-only LoRA merge into loaded transformer linear weights."""
...
def merge_flux1_lora_adapters(
    ...
    *,
    ...
    if not hasattr(model, "_param_map"):
        raise RuntimeError("Flux1 LoRA merge requires a TransformerBase-style model with ``_param_map``.")
    ...
    for item in adapters:
        ...
        groups = remap_flux1_lora_keys(weights)
        if not groups:
            raise RuntimeError(
        ...
        if applied == 0:
            raise RuntimeError(
                ...
                "this transformer (wrong base checkpoint / architecture)."
            )
        if on_log:
