"""Lightweight DiffRhythm 2 output quality heuristics."""
...
class DiffRhythmQualityAssessment:
    score: float
    grade: str
    warnings: tuple[str, ...]
...
def assess_generation_quality(
    *,
...
) -> DiffRhythmQualityAssessment:
    """Score 0–100 from signals collected during inference."""
    score = 78.0
    warnings: list[str] = []
    ...
    if score >= 72.0:
        ...
        grade = "poor"
    return DiffRhythmQualityAssessment(score=score, grade=grade, warnings=tuple(warnings))


def quality_log_message(assessment: DiffRhythmQualityAssessment) -> Optional[str]:
