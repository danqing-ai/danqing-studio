...
from backend.engine.common.ops.attention import rotate_half
...
class VisionAttention(nn.Module):
    ...
    def _apply_rope(self, x: mx.array, cos: mx.array, sin: mx.array) -> mx.array:
        ...
        rotated = (x * cos_expanded) + (rotate_half(mx, x) * sin_expanded)
    ...
    def __call__(self, x: mx.array, position_embeddings=None, cu_seqlens=None) -> mx.array:
        ...
        if cu_seqlens is not None and len(cu_seqlens) > 2:
            ...
            for length in lengths:
                ...
                offset += length
                out = scaled_dot_product_attention_bhsd_mx(mx, q_chunk, k_chunk, v_chunk, scale=scale)
            ...
            attn_output = scaled_dot_product_attention_bhsd_mx(mx, q_4d, k_4d, v_4d, scale=scale)
