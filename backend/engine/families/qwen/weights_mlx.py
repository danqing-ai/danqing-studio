"""Qwen-Image 权重：HF/diffusers → DanQing MLX。

- Transformer：``remap_qwen_transformer_weights`` → 扁平 ``dit.*``（与 Pipeline ``remap_fn`` 对齐）。
- 文本编码器 / VAE：``apply_qwen_*_weights`` → 嵌套 dict，供 ``module.update()``。
"""
