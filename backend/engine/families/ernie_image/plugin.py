"""ERNIE-Image v3 ``FamilyPlugin`` factory (Phase 2 — phased create via session helpers)."""
...
def build_ernie_image_plugin(
    ...
    *,
    ...
    config = get_config_class("ernie_image")()
    spec = family_spec_from_config("ernie_image", config, media="image")
    return FamilyPlugin(
        family_id="ernie_image",
        spec=spec,
        backbone=ImagePluginBackbone(spec),
    )


def register_ernie_image_plugin() -> None:
    register_family("ernie_image", build_ernie_image_plugin)
