"""ACE-Step 5Hz LM FSM constrained decoding (ported from upstream, inference-only)."""
...
import numpy as np
from backend.engine.families.ace_step.lm.lm_constants import (
    VALID_LANGUAGES,
    KEYSCALE_NOTES,
...
# ==============================================================================
# Array helpers (processor always receives numpy arrays from both MLX and CUDA)
# ==============================================================================
...
class MetadataConstrainedLogitsProcessor(LogitsProcessor):
    ...
    def __init__(
        ...
        self.genres_vocab_path = genres_vocab_path or str(
            Path(__file__).resolve().parent / "data" / "genres_vocab.txt"
        )
    ...
    def _precompute_tokens(self):
        ...
        self.audio_code_mask: Optional[np.ndarray] = None
        # Inverse mask: block all non-audio-code tokens (for CODES_GENERATION state)
        self.non_audio_code_mask: Optional[np.ndarray] = None
    ...
    def _build_audio_code_mask(self):
        ...
        # Create mask array: 0 everywhere, -inf at audio code positions
        mask = np.zeros((1, self.vocab_size), dtype=np.float32)
        ...
        inverse_mask = np.full((1, self.vocab_size), float('-inf'), dtype=np.float32)
    ...
    def _apply_whitelist_inplace(self, scores: Any, allowed_tokens: List[int]) -> None:
        """
        Apply whitelist constraint inplace: only allow specified tokens, block all others.
        
        Args:
            scores: [1, vocab_size] numpy scores array to modify inplace
        ...
        saved_values = scores[0, list(allowed_tokens)].copy()
        scores[:] = float('-inf')
        scores[0, list(allowed_tokens)] = saved_values
    ...
    def _should_end_numeric_field(self, logits: Any, min_val: int, max_val: int) -> bool:
        """
        ...
        # Get probabilities (numpy softmax)
        probs = _softmax_np(logits)
        
        newline_prob = float(probs[0, self.newline_token]) if self.newline_token else 0
        ...
        max_digit_prob = max(float(probs[0, t]) for t in allowed_digits)
    ...
    def _should_end_text_field(self, logits: Any) -> bool:
        """
        ...
        max_other_prob = float(masked_probs[0].max())
    ...
    def __call__(
        ...
        if self.state == FSMState.COMPLETED:
            ...
            if self.generation_phase == "understand" and self.audio_code_mask is not None:
                if self.audio_code_mask.dtype != scores.dtype:
                    self.audio_code_mask = self.audio_code_mask.astype(scores.dtype)
        ...
        elif self.generation_phase == "understand" and self.state == FSMState.THINK_TAG:
            if self._input_contains_think_end_tag(input_ids):
                self.state = FSMState.COMPLETED
                if self.debug:
                    logger.debug(
                        "Understand phase: detected </think> in input, skipping to lyrics"
                    )
        ...
        if self.state == FSMState.CODES_GENERATION:
            ...
            if self.non_audio_code_mask is not None:
                if self.non_audio_code_mask.dtype != scores.dtype:
                    self.non_audio_code_mask = self.non_audio_code_mask.astype(scores.dtype)
            ...
            if self.target_codes is not None and self.eos_token_id is not None:
                if self.codes_count < self.target_codes:
                    ...
                    eos_scores = scores[:, self.eos_token_id].copy()
                    scores[:] = float('-inf')
    ...
    def _input_contains_think_end_tag(self, input_ids: np.ndarray) -> bool:
        """
    ...
    def _process_single_sequence(
        ...
        if self.state in self.fixed_strings:
            ...
            if self.caption_after_newline:
                ...
                top_token_id = int(np.argmax(scores[0]))
            ...
            if not self.accumulated_token_ids:
                ...
                if empty_prefix in self.language_prefix_tree:
                    ...
                    if candidate_tokens:
                        ...
                        # Use array indexing to get scores of candidate tokens directly
                        candidate_scores = scores[0, candidate_tokens]
