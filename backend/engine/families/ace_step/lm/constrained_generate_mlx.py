...
from backend.engine.families.ace_step.lm.constrained_lm import MetadataConstrainedLogitsProcessor
...
def generate_constrained_mlx(
    ...
    *,
    ...
    return _generate_constrained_mlx_ar(
        model,
        tokenizer,
        prompt,
        processor,
...
def _generate_constrained_mlx_ar(
    model: Any,
    tokenizer: Any,
    prompt: str,
    processor: MetadataConstrainedLogitsProcessor,
    ...
    *,
    ...
    """Single-cache constrained decode via ``AutoregressiveInference`` (L2)."""
    import mlx.core as mx
    from mlx_lm.models.cache import make_prompt_cache
    from mlx_lm.sample_utils import make_sampler
    ...
    def prefill_fn(_prompt_tokens: list[int]) -> tuple[Any, dict[str, Any]]:
        remaining = prompt_mx
    ...
    def process_logits_fn(logits: Any, all_ids: list[int], _step_idx: int) -> Any:
        ...
        return mx.array(_log_softmax_np(step_logits, cfg.temperature))

    def sample_fn(logits: Any) -> int:
        token_arr = sampler(logits)
        eval_fn(token_arr)
    ...
    def logits_fn(token_id: int, state: dict[str, Any]) -> Any:
        next_input = mx.array([[token_id]])
        logits = model(next_input, cache=state["cache"])
        eval_fn(logits)
        return logits
