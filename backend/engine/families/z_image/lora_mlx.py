"""Z-Image / Z-Image-Turbo — MLX-only LoRA merge into loaded transformer linear weights."""
...
def merge_z_image_lora_adapters(
    ...
    *,
    ...
    if not hasattr(model, "_param_map"):
        raise RuntimeError("Z-Image LoRA merge requires a ZImageTransformer with ``_param_map``.")
    ...
    for item in adapters:
        ...
        groups = remap_zimage_lora_keys(weights, patch_size=ps)
        if not groups:
            raise RuntimeError(
        ...
        if applied == 0:
            raise RuntimeError(
                ...
                "this Z-Image transformer (wrong base_model / key layout)."
            )
        if on_log:
