"""BooguImageTransformer2DModel (OmniGen2-lineage) — MLX inference core."""
...
def rope_cos_sin(position_ids: np.ndarray, axes_dim=(40, 40, 40), theta: int = 10000):
    cos_parts, sin_parts = [], []
    for _a, dim in enumerate(axes_dim):
        inv_freq = 1.0 / (theta ** (np.arange(0, dim, 2, dtype=np.float64) / dim))
        ang = position_ids[:, _a : _a + 1].astype(np.float64) * inv_freq[None, :]
        cos_parts.append(np.cos(ang))
        sin_parts.append(np.sin(ang))
    cos = np.concatenate(cos_parts, axis=-1).astype(np.float32)
...
class BooguImageDiTMLX(nn.Module):
    def __init__(
        self,
        patch_size=2,
        in_channels=16,
        ...
        self.noise_refiner = [
            BasicBlock(hidden_size, h, kv, multiple_of, norm_eps, True)
        ...
        self.context_refiner = [
            BasicBlock(hidden_size, h, kv, multiple_of, norm_eps, False)
            for _ in range(num_refiner_layers)
        ]
        self.double_stream_layers = [
            DoubleStreamBlock(hidden_size, h, kv, multiple_of, norm_eps)
    ...
    @classmethod
    def from_config(cls, cfg: dict) -> "BooguImageDiTMLX":
        ...
        return cls(
            ...
            num_double_stream_layers=cfg["num_double_stream_layers"],
            num_refiner_layers=cfg["num_refiner_layers"],
            num_attention_heads=cfg["num_attention_heads"],
            num_kv_heads=cfg["num_kv_heads"],
            multiple_of=cfg["multiple_of"],
            norm_eps=cfg["norm_eps"],
            axes_dim_rope=cfg["axes_dim_rope"],
            axes_lens=cfg["axes_lens"],
            instruction_feat_dim=ic["instruction_feat_dim"],
    ...
    def _patchify(self, latent: mx.array) -> mx.array:
        ...
        p = self.patch_size
        ht, wt = h // p, w // p
        x = latent.reshape(b, c, ht, p, wt, p)
    ...
    def _unpatchify(self, tokens: mx.array, h: int, w: int) -> mx.array:
        ...
        ht, wt = h // p, w // p
        x = tokens.reshape(b, ht, wt, p, p, c)
    ...
    def _position_ids(self, l_cap: int, ht: int, wt: int, ref=None) -> np.ndarray:
        ...
        if ref is not None:
            ...
            parts.append(
                np.stack(
                    [
                        np.full(rht * rwt, shift, dtype=np.int64),
                        np.repeat(np.arange(rht), rwt),
                        np.tile(np.arange(rwt), rht),
                    ],
                    axis=1,
                )
            )
        ...
        parts.append(
            np.stack(
                [
                    np.full(ht * wt, shift, dtype=np.int64),
                    np.repeat(np.arange(ht), wt),
                    np.tile(np.arange(wt), ht),
                ],
                axis=1,
            )
        )
    ...
    def __call__(
        ...
        x = self.x_embedder(self._patchify(latent))
        ...
        l_cap_slice = full_cos[:, :l_cap]
        cap_cos, cap_sin = l_cap_slice, full_sin[:, :l_cap]
        ...
        if ref is not None:
            ...
            img = mx.concatenate([ref_tok, x], axis=1)
        else:
            for layer in self.noise_refiner:
