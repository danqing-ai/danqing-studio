"""LongCat-Video ``FamilyPlugin`` factory."""
...
def build_longcat_plugin(
    ...
    *,
    ...
    config = get_config_class("longcat")()
    spec = family_spec_from_config("longcat", config, media="video")
    return FamilyPlugin(family_id="longcat", spec=spec, backbone=VideoPluginBackbone(spec))


def register_longcat_plugin() -> None:
    register_family("longcat", build_longcat_plugin)
