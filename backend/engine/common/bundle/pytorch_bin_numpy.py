...
def _storage_array(raw: bytes, storage_type: Any, numel: int) -> np.ndarray:
    name = getattr(storage_type, "__name__", str(storage_type))
    if "BFloat16" in name:
        u16 = np.frombuffer(raw, dtype=np.uint16, count=int(numel)).copy()
...
class _TorchBinUnpickler(pickle.Unpickler):
    ...
    def persistent_load(self, pid: Any) -> _NumpyStorage:
        ...
        arr = _storage_array(raw, storage_type, int(numel))
