"""
Z-Image Transformer — 参考 mflux models/z_image 实现。

架构: ZImageAttention + FeedForward (SiLU-gated) + AdaLN modulation
      + RopeEmbedder (complex freq) + TimestepEmbedder
      + noise_refiner / context_refiner / main layers
      + FinalLayer (AdaLN + Linear output)

参数来自 ZImageConfig dataclass。
"""
...
class RopeEmbedder:
    """预计算 RoPE 频率 (复数形式 cos/sin stacked)。

    参考 mflux RopeEmbedder: axes_dims=[32,48,48], theta=256.0。
    """
    ...
    def __init__(self, config_or_dims: ZImageConfig | list[int] = None,
        ...
        if isinstance(config_or_dims, ZImageConfig):
            ...
            # 参考 mflux 默认: axes_lens=[1024, 512, 512]
    ...
    def forward(self, ids):
        ...
        for i in range(len(self.axes_dims)):
            index = ids[:, i].astype(self.ctx.int32() if hasattr(self.ctx, 'int32') else ids[:, i].dtype)
...
class TimestepEmbedder:
    """正弦时间步嵌入 + MLP。参考 mflux 实现，使用 mlx 原生函数确保数值一致。"""
...
class FeedForward:
    ...
    参考 mflux FeedForward: hidden_dim = int(dim / 3 * 8)
    """
...
class ZImageAttention:
    """Z-Image 自注意力，QK 分别投影 + QK Norm + 复数 RoPE。

    参考 mflux ZImageAttention: dim=3840, n_heads=30。
    """
...
class ZImageTransformerBlock:
    """主 Transformer 块：AdaLN 时间调制 (4 params: scale/gate for attn+ffn)。

    参考 mflux ZImageTransformerBlock。
    """
...
class FinalLayer:
    """输出层：LayerNorm + AdaLN scale + Linear → patch embeddings。
...
class ZImageTransformer(TransformerBase):
    ...
    def load_weights(self, weights: list[tuple[str, Any]], strict: bool = False):
        ...
        for key, param in list(self._param_map.items()):
            if param.dtype == mx.bfloat16 or param.dtype not in (mx.float32, mx.float16):
                continue
            ...
            last = parts[-1]
            if hasattr(obj, last): setattr(obj, last, new_param)
    ...
    def forward(self, latents, timestep,
        ...
        # Time embedding — 参考 mflux ZImageTransformer.__call__
        # 若 timestep 为整数索引，需通过 sigmas 查表；若为标量则视为 sigma 值
        import mlx.core as mx
        t = timestep
        # Pipeline 统一传入 int index，由模型自己转换
        if isinstance(t, int) or (hasattr(t, 'dtype') and t.dtype in (mx.int32, mx.int64)):
            if sigmas is None:
                raise ValueError("ZImageTransformer requires sigmas when timestep is an integer index")
            sigma_t = sigmas[int(t)].reshape((1,))
            t = mx.ones_like(sigma_t) - sigma_t
        else:
        ...
        t_emb = self.t_embedder.forward(t.astype(mx.float32) * self.t_scale)
...
def _collect_nn_params(obj, prefix: str, result: dict):
    """递归收集所有 MLX nn.Module 的参数到 result[full_path] = param_tensor。"""
