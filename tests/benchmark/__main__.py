"""``python -m tests.benchmark mflux|sanity|diffusers [--all|--case ID]``"""
...
from tests.benchmark.run import run_diffusers, run_mflux, run_sanity


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="DanQing benchmark (mflux | sanity | diffusers)"
    )
    ...
    for name, list_fn, help_text in suite_defs:
        ...
        p.add_argument(
            "--list-runnable",
            action="store_true",
            help="(mflux/diffusers) List runnable cases with local bundles",
        )
        p.add_argument("--output-dir", default="tests/benchmark/outputs")

    args = parser.parse_args(argv)
    if getattr(args, "list_runnable", False):
        if args.suite not in ("mflux", "diffusers"):
            parser.error("--list-runnable is only for mflux/diffusers suites")
